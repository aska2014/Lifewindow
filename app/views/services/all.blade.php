...
<div id="main">	
	...
	<div class="wrapper clearfix">
		...
		<!-- pager -->
		<ul class="pager">
			<li class="paged">{{ Lang::get('words.page_1_of') }} {{ $services->getLastPage() }}</li>
			@for( $i = 1; $i <= $services->getLastPage(); $i++ )
		
				@if($i == $services->getCurrentPage())
					<li class="active">
				@else
					<li>
				@endif
		
				<a href="{{ $services->getUrl($i) }}">{{ $i }}</a></li>
			@endfor
		</ul>
